...
class Evaluator:
    ...
    def _eval_func(self, name: str, arg_nodes: list[Expr], row: int, col: int) -> Any:
        from vimsheet.formula.functions.registry import get as registry_get
        ...
        if getattr(fn, "_is_script_func", False) and arg_nodes:
            from vimsheet.formula.ast_nodes import CellRefNode, RangeRefNode
        ...
        if name == "FETCH":
            import vimsheet.formula.functions.net_funcs as _nf
            from vimsheet.fetch.fetch_manager import _get_global_manager
