"""Lesson data generators for the interactive tutorial system.

Each ``lesson_NN()`` function returns a sheet dict.  Use ``generate_lesson(num)``
to get a complete workbook dict, or ``generate_all()`` for a filename → dict map.
"""
...
def lesson_15() -> dict:
    ...
    steps = [
        ...
        (
            5,
            "3. Go to row 6 (WIDGET PRO). Type\n:s/WIDGET/Widget/i Enter — /i = case-insensitive.",
        ),
        (
            6,
            "4. Go to column B. Type :cs/gadget/Gadget/i Enter.\ncs = column substitute; applies to all cells in the column.",  # noqa: E501
        ),
        (
            7,
        ...
        (
            8,
            "6. Rows 9–11 now read new-part-A/B/C.\nRange-substitute applied to each selected row.",
        ),
        (
            9,
...
def generate_lesson(num: int) -> dict:
    """Generate a single lesson and return a complete workbook dict."""
    sheet = _GENERATORS[num]()
...
def generate_all() -> dict[str, dict]:
    """Generate all 21 lessons.

    Returns a dict mapping ``"<filename>.vimsheet"`` → workbook dict.
    """
    return {f"{fname}.vimsheet": generate_lesson(num) for num, fname, _ in LESSONS}
